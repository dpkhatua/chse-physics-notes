...
chapter("ch01", 1, "Electrostatics",
  ...
  sections=[
    ...
    section("10 Model Questions — Fully Solved", [
      p("These are worked in full, exam-answer style, from the textbook's own Model Questions for this chapter "
        "— 8 numericals and 2 long-answer conceptual questions. Cover the answer and try each one yourself first."),
      solved(
        "1. The distance between the electron and proton in a hydrogen atom is 0.53 Å. Find the speed and "
        "frequency of revolution of the electron round the proton. (mₑ = 9.1 × 10⁻³¹ kg)",
        [
          "The Coulomb force of attraction provides the centripetal force for the electron's circular orbit: ke²/r² = mv²/r",
          "So v² = ke²/(mr) = (9×10⁹ × (1.6×10⁻¹⁹)²) / (9.1×10⁻³¹ × 0.53×10⁻¹⁰)",
          "v² = 2.304×10⁻²⁸ / 4.823×10⁻⁴¹ = 4.78×10¹² → v = √(4.78×10¹²)",
          "Frequency of revolution: f = v / (2πr)",
        ],
        "v ≈ 2.19 × 10⁶ m/s, f ≈ 6.56 × 10¹⁵ Hz",
      ),
      solved(
        "2. Two identical charges, placed 1.0 m apart, repel each other with a force of 36 kgwt. Find the "
        "magnitude of each charge. (g = 10 m/s²)",
        [
          "Convert the force to newtons: F = 36 kgwt = 36 × 10 = 360 N",
          "Coulomb's law: F = kq²/r² → q² = Fr²/k = (360 × 1²) / (9×10⁹)",
          "q² = 4 × 10⁻⁸ → q = √(4×10⁻⁸)",
        ],
        "q = 2 × 10⁻⁴ C = 200 μC on each charge",
      ),
      solved(
        "3. Find the Coulomb force between two protons in a nucleus if the average distance between protons in "
        "the nucleus is 6 fermi (1 fermi = 10⁻¹⁵ m).",
        [
          "F = kq²/r², with q = e = 1.6×10⁻¹⁹ C and r = 6×10⁻¹⁵ m",
          "F = (9×10⁹ × (1.6×10⁻¹⁹)²) / (6×10⁻¹⁵)² = (9×10⁹ × 2.56×10⁻³⁸) / (3.6×10⁻²⁹)",
          "F = 2.304×10⁻²⁸ / 3.6×10⁻²⁹",
        ],
        "F ≈ 6.4 N — notice how enormous this is at nuclear scale, which is exactly why the (stronger) nuclear force is needed to hold the nucleus together against this repulsion.",
      ),
      solved(
        "4. Two charged particles are a distance 1 mm apart. Find the minimum possible Coulomb force between "
        "them.",
        [
          "The smallest possible charge on any body is the elementary charge, e = 1.6×10⁻¹⁹ C — so the minimum "
          "force occurs when both particles carry exactly this charge.",
          "F = ke²/r² = (9×10⁹ × (1.6×10⁻¹⁹)²) / (10⁻³)² = (9×10⁹ × 2.56×10⁻³⁸) / 10⁻⁶",
        ],
        "F(min) ≈ 2.3 × 10⁻²² N",
      ),
      solved(
        "5. A glass ball is rubbed with an ebonite ball and the two are then kept 1 cm apart. If the force of "
        "attraction between them is 10 N, find the charge on each ball.",
        [
          "Coulomb's law: F = kq²/r² → q² = Fr²/k",
          "q² = (10 × (0.01)²) / (9×10⁹) = (10 × 10⁻⁴) / (9×10⁹) = 1.11×10⁻¹³",
          "q = √(1.11×10⁻¹³)",
        ],
        "q ≈ 0.33 μC on each ball (glass positive, ebonite negative, since it's an attraction)",
      ),
      solved(
        "6. Three equal charges, each of 1 μC, are placed at the three corners of an equilateral triangle of "
        "side 1 m. Find the force experienced by each charge due to the other two.",
        [
          "Force between any one pair: F(pair) = kq²/r² = (9×10⁹ × (10⁻⁶)²) / 1² = 9×10⁻³ N",
          "From the general result derived earlier (§ Superposition), the resultant on any one charge in this "
          "symmetric arrangement is F(net) = √3 × F(pair)",
          "F(net) = √3 × 9×10⁻³",
        ],
        "F(net) ≈ 1.56 × 10⁻² N, directed radially outward along the bisector from that corner",
      ),
      solved(
        "7. Four equal charges, each 1 μC, are placed at the four corners of a square of side 3 cm. Find the "
        "force experienced by each charge due to the other three.",
        [
          "Force from each adjacent charge (distance = side a = 0.03 m): F(adj) = kq²/a² = (9×10⁹×10⁻¹²)/(9×10⁻⁴) = 10 N — and there are two such adjacent charges.",
          "Force from the diagonal charge (distance = a√2 = 0.0424 m): F(diag) = kq²/(2a²) = 9×10⁻³/1.8×10⁻³ = 5 N",
          "The two adjacent forces are perpendicular; their resultant = √2 × 10 = 14.14 N, directed along the diagonal (same direction as the diagonal force).",
          "Total force = 14.14 + 5 (both act along the same diagonal, pointing away from the square's centre)",
        ],
        "F(net) ≈ 19.1 N, directed outward along the diagonal",
      ),
      solved(
        "8. A spherical conductor carrying a charge of 100 μC is brought near an identical metallic sphere of "
        "mass 36 kg resting on the ground. Assuming the second sphere acquires an equal charge of 100 μC, what "
        "minimum separation must be maintained so the electrostatic force can just lift it? (g = 10 m/s²)",
        [
          "At the minimum separation, the Coulomb force just balances gravity: kQ²/r² = mg",
          "r² = kQ²/(mg) = (9×10⁹ × (10⁻⁴)²) / (36 × 10) = (9×10⁹ × 10⁻⁸) / 360 = 90/360 = 0.25",
          "r = √0.25",
        ],
        "r = 0.5 m. (If the separation is larger than this, the sphere cannot be lifted; if smaller, the electrostatic force exceeds gravity and the sphere accelerates upward.)",
      ),
      solved(
        "9. [Long answer] State the principle of superposition in electrostatics. Obtain an expression for the "
        "force on one charge due to all other charges present in its neighbourhood.",
        [
          "<b>Statement:</b> the principle of superposition states that the force between any two charges in a "
          "group is unaffected by the presence of other charges — the net force on any one charge is the vector "
          "sum of the individual Coulomb forces due to every other charge, taken one pair at a time.",
          "<b>Setup:</b> consider a charge Q at the origin, with charges Q1, Q2, ... Qn at position vectors "
          "r1, r2, ... rn from it.",
          "By Coulomb's law, the force on Q due to Q1 alone is F1 = (1/4πε0)(QQ1/r1²)r̂1 — and similarly for "
          "F2, F3, ... Fn, each acting exactly as if the others weren't present.",
          "By the superposition principle, the total force is simply the vector sum of all these individual forces.",
        ],
        "F = F1 + F2 + ... + Fn = (Q/4πε0) Σ (Qi/ri²) r̂i — the vector sum of all pairwise Coulomb forces.",
      ),
      solved(
        "10. [Long answer] What do you mean by electrification? Discuss the various ways of electrifying a "
        "body. How can a conductor be charged positively by induction?",
        [
          "<b>Electrification</b> is the process of charging an originally uncharged (neutral) body.",
          "<b>By friction:</b> rubbing transfers electrons from one body to the other — the body losing "
          "electrons becomes positive, the one gaining electrons becomes negative (e.g. glass rubbed with silk).",
          "<b>By conduction:</b> touching a charged conductor to an uncharged one transfers some charge of the "
          "<i>same sign</i> to the second conductor, the amount depending on their relative sizes.",
          "<b>By induction:</b> bringing a charged body near (without touching) a conductor redistributes its "
          "free charges — the near end gets an induced charge <i>opposite</i> to the inducing body, the far end "
          "gets the <i>same</i> sign.",
          "<b>To charge a conductor positively by induction:</b> take two touching, uncharged conducting spheres. "
          "Bring a <i>negatively</i> charged rod close to the sphere nearer to it (without touching). The rod "
          "repels free electrons in the conductor toward the far sphere, leaving the near sphere with a deficit "
          "of electrons — i.e. positively charged. While the rod is still in place, separate the two spheres. "
          "Finally remove the rod: the near sphere retains its positive charge permanently, and the far sphere "
          "retains an equal negative charge.",
        ],
        "The near sphere ends up permanently positively charged — induction always leaves the sphere closer to the inducing (negative) rod with the opposite (positive) charge.",
      ),
    ]),
  ],
  recap=[
